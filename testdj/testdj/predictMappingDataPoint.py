...
class PredictMapDataPoint:
    ...
    def predictInfo(self,modelItem,fileInfo):
        ...
        #load 词向量模型
        fileVector = open( fileRootPath + '/matchDataPoint/vectorizer_matchDataPoint_wordModel.pkl','rb')
        ...
        #load 词向量模型
        labelFilePath = open(fileRootPath + '/matchDataPoint/vectorizer_matchDataPoint_labelModel.pkl','rb')
    ...
    def createContentInfo(self,strArray):
        ...
        if strArray != None and len(strArray) > 0:
            for j in strArray:
                j = j.lower()
                j = strUtil.removeStopWord_title(j)
    ...
    def getPredictInfo(self,modelItem,content):

        contentArray = [content]
        contentInfo = self.createContentInfo(contentArray)
        predictLabel,accuracy =self.predictInfo(modelItem,contentInfo)
